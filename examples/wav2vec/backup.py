...
while True:
    ...
    run("aws s3 cp results/checkpoint_last.pt s3://dl4s-datasets/01-backup/", shell=True)
    run("aws s3 cp results/hydra_train.log s3://dl4s-datasets/01-backup/", shell=True)
    run("aws s3 cp results/graphs.zip s3://dl4s-datasets/01-backup/", shell=True)
