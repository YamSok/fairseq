...
while True:
    ...
    run(f"aws s3 cp results/checkpoint_last.pt s3://dl4s-datasets/0{args.dest}-backup", shell=True)
    run(f"aws s3 cp results/hydra_train.log s3://dl4s-datasets/0{args.dest}-backup/", shell=True)
    run(f"aws s3 cp results/graphs.zip s3://dl4s-datasets/0{args.dest}-backup", shell=True)
